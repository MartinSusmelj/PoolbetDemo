// v8: disable Join for LOCKED + RESOLVED; use Lightweight-Charts widget (no external links) with guide lines.
...
// seed history
for(const c of COINS){const base=PRICE_BASE[c];PRICE_HISTORY[c]=[];let v=base;for(let i=240;i>0;i--){v=Math.max(0.0001,v+(Math.random()-0.5)*base*0.002);PRICE_HISTORY[c].push({time:nowSec()-i*60,value:+v.toFixed(4)});}}
...
const modalJoin=document.getElementById('modal-join');const joinPoolName=document.getElementById('join-pool-name');const joinPoolInfo=document.getElementById('join-pool-info');const betAmount=document.getElementById('bet-amount');const betPrice=document.getElementById('bet-price');const betConfirm=document.getElementById('bet-confirm');const betMsg=document.getElementById('bet-msg');document.getElementById('modal-join-close').onclick=()=>modalJoin.classList.add('hidden');document.getElementById('bet-cancel').onclick=()=>modalJoin.classList.add('hidden');
...
const modalStatus=document.getElementById('modal-status');const statusTitle=document.getElementById('status-title');const statusBody=document.getElementById('status-body');document.getElementById('modal-status-close').onclick=()=>modalStatus.classList.add('hidden');
...
function drawStatusChart(p){const el=document.getElementById('status-chart');el.innerHTML='';const chart=LightweightCharts.createChart(el,{layout:{background:{type:'solid',color:'#0F1630'},textColor:'white'},grid:{horzLines:{color:'rgba(255,255,255,0.06)'},vertLines:{color:'rgba(255,255,255,0.06)'}},rightPriceScale:{borderColor:'rgba(255,255,255,0.2)'},timeScale:{borderColor:'rgba(255,255,255,0.2)'}});const line=chart.addLineSeries();const data=(PRICE_HISTORY[p.coin]||[]).map(d=>({time:d.time,value:d.value}));line.setData(data); // guide lines
  // Target line
  line.createPriceLine({ price: p.targetPrice, color:'#7C93F6', lineWidth:2, lineStyle:0, axisLabelVisible:true, title:'Target' });
  // Your prediction (if exists)
  const your = p.participants.find(x=>x.user===USER.pubkey);
  if(your){ line.createPriceLine({ price: your.predictedPrice, color:'#22d3ee', lineWidth:2, lineStyle:2, axisLabelVisible:true, title:'Your prediction' }); }
  // Final (if resolved): draw after a short tick to ensure rendering
  if(p.status==='RESOLVED'){ setTimeout(()=>{ line.createPriceLine({ price: PRICE[p.coin], color:'#ef4444', lineWidth:2, lineStyle:1, axisLabelVisible:true, title:'Final' }); }, 0); }
  function resize(){chart.applyOptions({width:el.clientWidth,height:240});}window.addEventListener('resize',resize);resize();}
...
setInterval(()=>{for(const c of COINS){const b=PRICE_BASE[c];const vol=b*0.0015;PRICE[c]=Math.max(0.0001,PRICE[c]+(Math.random()-0.5)*2*vol);PRICE_HISTORY[c].push({time:nowSec(),value:+PRICE[c].toFixed(4)});if(PRICE_HISTORY[c].length>480)PRICE_HISTORY[c].shift();}},2000);
