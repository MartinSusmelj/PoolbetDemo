// v10: Create Pool coin-reactive chart; themed charts; per-participant Expected/Final payout in Status.
...
// theme colors per coin
const COIN_COLORS={
  BTC:'#f59e0b', ETH:'#6366f1', SOL:'#22d3ee', ARB:'#10b981',
  AVAX:'#ef4444', DOGE:'#a3e635', MATIC:'#8b5cf6', ADA:'#60a5fa'
};
...
async function connectWallet(){const p=window.solana;if(!p||!p.isPhantom){USER.connected=true;USER.pubkey='DEMO-'+Math.random().toString(36).slice(2,8).toUpperCase();walletInfo.textContent=USER.pubkey;btnConnect.textContent=USER.pubkey.slice(0,6)+'...';showPopup('Demo wallet connected. Credits: $'+USER.balance.toFixed(2));updateUserStatsUI();renderPools();setupCreateChart();return;}try{const r=await p.connect();USER.connected=true;USER.pubkey=r.publicKey.toString();walletInfo.textContent=USER.pubkey.slice(0,10);btnConnect.textContent=USER.pubkey.slice(0,6)+'...';showPopup('Wallet connected: '+USER.pubkey.slice(0,6));updateUserStatsUI();renderPools();setupCreateChart();}catch(e){console.log('connect cancelled',e);}}
...
let cpChart=null;const cpChartElId='cp-chart';
function setupCreateChart(){const coinSel=document.getElementById('cp-coin');if(!coinSel) return;const el=document.getElementById(cpChartElId);function draw(){if(cpChart){cpChart.destroy();cpChart=null;}const coin=coinSel.value;const color=COIN_COLORS[coin]||'#7C93F6';const d=(PRICE_HISTORY[coin]||[]).slice(-120);cpChart=new Chart(el.getContext('2d'),{type:'line',data:{labels:d.map(x=>new Date(x.t*1000).toLocaleTimeString()),datasets:[{label:`${coin} price`,data:d.map(x=>x.y),fill:false,tension:0.25,borderColor:color}]},options:{responsive:true,plugins:{legend:{display:false},tooltip:{enabled:true}},scales:{x:{display:false,grid:{display:false}},y:{display:true,grid:{color:'rgba(255,255,255,0.08)'}}}});}coinSel.onchange=draw;draw();}
...
function drawJoinChart(p){if(joinChart){joinChart.destroy();joinChart=null;}const coin=p.coin;const color=COIN_COLORS[coin]||'#7C93F6';const d=(PRICE_HISTORY[coin]||[]).slice(-120);const labels=d.map(x=>new Date(x.t*1000).toLocaleTimeString());const data=d.map(x=>x.y);joinChart=new Chart(joinChartEl.getContext('2d'),{type:'line',data:{labels,datasets:[{label:`${coin} price`,data,fill:false,tension:0.25,borderColor:color}]},options:{responsive:true,plugins:{legend:{display:false},tooltip:{enabled:true}},scales:{x:{display:false,grid:{display:false}},y:{display:true,grid:{color:'rgba(255,255,255,0.08)'}}}});
  // draw target overlay
  setTimeout(()=>{const y=joinChart.scales.y.getPixelForValue(p.targetPrice);const ctx=joinChart.ctx;ctx.save();ctx.strokeStyle='rgba(124,147,246,0.9)';ctx.setLineDash([4,4]);ctx.beginPath();ctx.moveTo(joinChart.chartArea.left,y);ctx.lineTo(joinChart.chartArea.right,y);ctx.stroke();ctx.fillStyle='rgba(124,147,246,0.9)';ctx.fillText('Target $'+p.targetPrice, joinChart.chartArea.left+6, y-4);ctx.restore();},0);
}
...
function openStatusModal(id){const p=POOLS.find(x=>x.id===id);if(!p)return;statusTitle.textContent=`Status – ${p.name}`;const price=PRICE[p.coin];const pot=p.participants.reduce((s,a)=>s+a.amount,0);
  // Expected now per participant
  let bestErr=Infinity;p.participants.forEach(pt=>{bestErr=Math.min(bestErr,Math.abs(price-pt.predictedPrice));});
  ...
  const rows = p.participants.length ? p.participants.map(pt=>{
    const isNow = winnersNow.includes(pt);
    const expected = isNow ? ('$'+shareNow.toFixed(2)) : '$0.00';
    const final = (p.status==='RESOLVED') ? ('$'+((p.payouts && p.payouts[pt.user]) || 0).toFixed(2)) : '—';
    return `<tr>
      ...
      <td class="py-1 small">${expected}</td>
      <td class="py-1 small">${final}</td>
    </tr>`;
  }).join('') : `<tr><td class="py-2 small muted" colspan="5">No participants yet</td></tr>`;
  ...
  statusBody.innerHTML = `
    <div class="grid md:grid-cols-3 gap-3">
      ...
      <div class="card p-3">
        ...
        <div class="small muted mt-2">Now-leading predictions</div><div class="font-semibold">${winnersNow.length}</div>
      </div>
      <div class="card p-3 md:col-span-1">
        <div class="small muted mb-2">Participants</div>
        <table class="w-full">
          <thead><tr class="small muted"><th class="text-left">User</th><th class="text-left">Amount</th><th class="text-left">Prediction</th><th class="text-left">Expected now</th><th class="text-left">Final payout</th></tr></thead>
          <tbody>${rows}</tbody>
        </table>
      </div>
    </div>
  ...
  modalStatus.classList.remove('hidden');drawStatusChart(p);
}

function drawStatusChart(p){if(statusChart){statusChart.destroy();statusChart=null;}const coin=p.coin;const color=COIN_COLORS[coin]||'#7C93F6';const d=(PRICE_HISTORY[coin]||[]).slice(-240);const labels=d.map(x=>new Date(x.t*1000).toLocaleTimeString());const data=d.map(x=>x.y);statusChart=new Chart(statusChartEl.getContext('2d'),{type:'line',data:{labels,datasets:[{label:`${coin} price`,data,fill:false,tension:0.25,borderColor:color}]},options:{responsive:true,plugins:{legend:{display:false},tooltip:{enabled:true}},scales:{x:{display:false,grid:{display:false}},y:{display:true,grid:{color:'rgba(255,255,255,0.08)'}}}});
  // overlays
  setTimeout(()=>{const ctx=statusChart.ctx;const yScale=statusChart.scales.y;function hLine(val, label, style='rgba(124,147,246,0.9)', dash=[]){const y=yScale.getPixelForValue(val);ctx.save();ctx.strokeStyle=style;ctx.setLineDash(dash);ctx.beginPath();ctx.moveTo(statusChart.chartArea.left,y);ctx.lineTo(statusChart.chartArea.right,y);ctx.stroke();ctx.fillStyle=style;ctx.fillText(label+' $'+val.toFixed(2), statusChart.chartArea.left+6, y-4);ctx.restore();}
    hLine(p.targetPrice,'Target','#7C93F6',[4,4]);
    const your=p.participants.find(x=>x.user===USER.pubkey);
    if(your) hLine(your.predictedPrice,'Your prediction','#22d3ee',[6,4]);
    if(p.status==='RESOLVED') hLine(PRICE[p.coin],'Final','#ef4444',[2,3]);
  },0);
}

const modalCreate=document.getElementById('modal-create');document.getElementById('btn-create').onclick=()=>{modalCreate.classList.remove('hidden');setupCreateChart();};document.getElementById('modal-create-close').onclick=()=>modalCreate.classList.add('hidden');document.getElementById('cp-cancel').onclick=()=>modalCreate.classList.add('hidden');document.getElementById('cp-create').onclick=()=>{const name=document.getElementById('cp-name').value.trim();const coin=document.getElementById('cp-coin').value;const price=parseFloat(document.getElementById('cp-price').value);const dt=document.getElementById('cp-datetime').value;const buyin=parseFloat(document.getElementById('cp-buyin').value);const win=parseInt(document.getElementById('cp-window').value,10);if(!name||!coin||!price||!dt||!buyin||!win){document.getElementById('cp-msg').textContent='Fill all fields';return;}const id='POOL-'+Math.random().toString(36).slice(2,7).toUpperCase();const regEnd=futureMs(win);const lock=regEnd+5*60*1000;const resolve=lock+5*60*1000;const pool={id,name,coin,minBuy:buyin,buyIn:0,entries:0,status:'OPEN',registrationEnd:regEnd,lockTime:lock,resolveTime:resolve,targetPrice:price,participants:[],creator:USER.pubkey,winners:[],payouts:{}};POOLS.unshift(pool);renderPools();modalCreate.classList.add('hidden');showPopup(`✅ Pool created: ${name} (${coin})`);const count=3+Math.floor(Math.random()*8);const duration=Math.max(1000,regEnd-Date.now());const step=Math.max(800,Math.floor(duration/count));let joined=0;const iv=setInterval(()=>{if(Date.now()>=regEnd||joined>=count){clearInterval(iv);return;}const amt=[5,10,15,20,25][Math.floor(Math.random()*5)];const pred=+(price*(0.97+Math.random()*0.06)).toFixed(2);pool.participants.push({user:'RND-'+Math.random().toString(36).slice(2,6),amount:amt,predictedPrice:pred});pool.entries++;pool.buyIn+=amt;joined++;renderPools();},step);};
...
// Update prices & history + refresh Create/Join charts lightly
setInterval(()=>{for(const c of COINS){const b=PRICE_BASE[c];const vol=b*0.0015;PRICE[c]=Math.max(0.0001,PRICE[c]+(Math.random()-0.5)*2*vol);PRICE_HISTORY[c].push({t:nowSec(),y:+PRICE[c].toFixed(4)});if(PRICE_HISTORY[c].length>480)PRICE_HISTORY[c].shift();}
  // refresh cp chart if open
  if(document.getElementById('modal-create') && !document.getElementById('modal-create').classList.contains('hidden')){setupCreateChart();}
  // refresh join chart if open
  if(document.getElementById('modal-join') && !document.getElementById('modal-join').classList.contains('hidden')){ /* re-draw on next open */ }
},2000);
